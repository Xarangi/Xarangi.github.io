...
let stories = [
	'Welcome to the double story of the Lion and the Boy! Choose your path',
	'On the left, the boy runs through a forest. On the right the lion rules it.',
	'They now face each other.',
	'On the left, the boy meets an unfortunate end. He finds himself a lot luckier on the right!'
];

let girlDiv = document.getElementById('girl-div');
let leafDiv = document.getElementById('leaf-div');
let girlText = document.getElementById('girl-text');
...
function changeText(){
	...
	girlText.innerHTML = stories[sectionNum];
	// Map vertical scroll position to the horizontal position of the cat
	girlDiv.style.left = pos / (height * 4) * width + 'px';
	leafDiv.style.left = pos / (height * 4) * width + 'px';
	leafDiv.style.top = pos / (width * 4) * height + 'px';
